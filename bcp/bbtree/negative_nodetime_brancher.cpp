...
struct AgentCandidate
{
    ...
    AgentCandidate(const Agent a, const Real64 val) :
        a(a),
        val(val)
    {
    }
    auto operator<=>(const AgentCandidate& rhs) const
    {
        return a <=> rhs.a;
    }
};
...
struct NodeCandidate
{
    ...
    NodeCandidate(const Node n, const Real64 val) :
        n(n),
        val(val)
    {
    }
    auto operator<=>(const NodeCandidate& rhs) const
    {
        return n <=> rhs.n;
    }
};
...
Decisions NegativeNodeTimeBrancher::branch()
{
    ...
    for (Agent a = 0; a < A; ++a)
    {
        ...
        for (auto& same_agent_candidates_a_t : same_agent_candidates_a)
        {
            ...
            for (Size64 idx = same_agent_candidates_a_t.size() - 2; idx >= 0; --idx)
            {
    ...
    for (auto& [nt, different_agent_candidates_nt] : different_agent_candidates)
    {
        ...
        for (Size64 idx = different_agent_candidates_nt.size() - 2; idx >= 0; --idx)
        {
            const auto a1 = different_agent_candidates_nt[idx].a;
            const auto a2 = different_agent_candidates_nt[idx + 1].a;
            if (a1 == a2)
            {
                different_agent_candidates_nt[idx].val +=
                    different_agent_candidates_nt[idx + 1].val;
                different_agent_candidates_nt.erase(different_agent_candidates_nt.begin() + idx +
                                                    1);
            }
        }
    }
    ...
    for (const auto& [nt, agent_candidates] : different_agent_candidates)
        for (auto it1 = agent_candidates.begin(); it1 != agent_candidates.end(); ++it1)
            for (auto it2 = it1 + 1; it2 != agent_candidates.end(); ++it2)
            {
                ...
                if ((val > best_val) ||
                    (val == best_val && num_paths[a1] + num_paths[a2] > best_num_paths))
                {
                    best_val = val;
    ...
    for (Agent a = 0; a < A; ++a)
        for (Time t = 0; t < same_agent_candidates[a].size(); ++t)
        {
            ...
            for (auto it1 = same_agent_candidates_a_t.begin();
                 it1 != same_agent_candidates_a_t.end();
                 ++it1)
    ...
    ASSERT(best_ant1->a >= 0, "Negative nodetime branching failed to make a decision");
    ASSERT((best_ant1->a != best_ant2->a && best_ant1->nt == best_ant2->nt) ||
               (best_ant1->a == best_ant2->a && best_ant1->nt.t == best_ant2->nt.t &&
                best_ant1->nt.n != best_ant2->nt.n),
           "Negative nodetime branching decided on an invalid branching - agent {} node {} time "
           "{}, agent {} node {} time {}",
           best_ant1->a,
           best_ant1->nt.n,
           best_ant1->nt.t,
...
void NegativeNodeTimeBrancher::add_pricing_costs(const BrancherData* const data)
{
    ...
    pricer.add_nodetime_penalty_one_agent(a, nt, COST_INF);
}
...
void NegativeNodeTimeBrancher::disable_vars(const BrancherData* const data)
{
    ...
    DEBUGLN("Negative nodetime brancher disabling paths incompatible with decision on agent {} "
            "forbidding {} in B&B node {}:",
            a,
...
void NegativeNodeTimeBrancher::print(const BrancherData* const data) const
{
    ...
    // Print.
    PRINTLN("    Agent {} forbidding {}", a, format_nodetime(nt, instance_.map));
}
