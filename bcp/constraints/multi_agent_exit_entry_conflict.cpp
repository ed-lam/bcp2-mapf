...
MultiAgentExitEntryConflictSeparator::MultiAgentExitEntryConflictSeparator(const Instance& instance,
                                                                           Problem& problem) :
...
void MultiAgentExitEntryConflictSeparator::separate()
{
    ...
    for (Agent canonical = 0; canonical < A; ++canonical)
        for (const auto& [canonical_et, et_val_canonical] :
            ...
            if (!is_integral(et_val_canonical))
            {
                ...
                const auto& [et_total, et_vals] =
                    projection.find_list_fractional_move_edgetime(canonical_et);
                ...
                const auto canonical_e = canonical_et.e();
                ...
                const auto& [origin_total, origin_vals] =
                ...
                const auto& [opposite_total, opposite_vals] =
                    projection.find_list_fractional_move_edgetime(opposite);
                const auto opposite_other = opposite_total - opposite_vals[canonical];

                // Compute part of the LHS, comprising the canonical agent's edge, all edges
                // outgoing from the origin, all edges incoming to the destination and subtracting
                // the duplicated edge.
                const auto lhs = 3.0 * et_val_canonical + origin_val_other + destination_val_other -
                                 et_val_other + opposite_other;
    ...
    {
        const auto& [lhs, num_other, canonical, t, canonical_e, opposite_e, origin, destination] =
            candidate;
...
void MultiAgentExitEntryConflictSeparator::create_row(
    const MultiAgentExitEntryConstraintCandidate& candidate)
{
    ZoneScopedC(TRACY_COLOUR);
    ...
    //         const auto& projection = problem_.projection();
    //         Real64 check_lhs =
    //             3 * projection.find_agent_move_edgetime(canonical, EdgeTime{canonical_e, t});
    //         for (Size64 idx = 0; idx < num_other_es; ++idx)
    //         {
    //             const auto& [total, agent] =
    //                 other_es[idx].d != Direction::WAIT ?
    //                     projection.find_list_fractional_move_edgetime(EdgeTime{other_es[idx], t})
    //                     : projection.find_list_fractional_wait_edgetime(EdgeTime{other_es[idx],
    //                     t});
    //             check_lhs += total - agent[canonical];
    //
    //             for (Size64 idx2 = idx + 1; idx2 < num_other_es; ++idx2)
...
#ifdef PRINT_DEBUG
    {
        ...
        DEBUGLN("    Creating multi-agent exit-entry conflict cut on {} for agent {} and {} for "
                "other agents with LHS {}",
    ...
    const auto data_size = sizeof(Agent) * 2 + sizeof(Time) * 1 + sizeof(Edge) * 1 +
                           sizeof(Size32) * 1 + sizeof(Edge) * num_other_es;
    const auto hash_size = data_size;
    auto constraint = Constraint::construct(
        '<', 3.0, 1, data_size, hash_size, &apply_in_pricer, &get_coeff, name);
    ...
    data->t = t;
    data->canonical_e = canonical_e;
    data->num_other_es = num_other_es;
    new (data->other_es) Edge[num_other_es];
    std::copy(other_es, other_es + num_other_es, data->other_es);
    master.add_row(std::move(constraint));
    ++num_added_;
}

void MultiAgentExitEntryConflictSeparator::apply_in_pricer(const Constraint& constraint,
                                                           const Real64 dual, Pricer& pricer)
{
    ZoneScopedC(TRACY_COLOUR);

    // Get the constraint data.
    const auto& [_, canonical, t, canonical_e, num_other_es, other_es] =
    ...
    for (Size64 index = 0; index < num_other_es; ++index)
    {
...
Real64 MultiAgentExitEntryConflictSeparator::get_coeff(const Constraint& constraint, const Agent a,
                                                       const Path& path)
{
    ZoneScopedC(TRACY_COLOUR);
...
Real64 MultiAgentExitEntryConflictSeparator::calculate_canonical_agent_coeff(const Edge e,
                                                                             const Time t,
                                                                             const Path& path)
{
...
Real64 MultiAgentExitEntryConflictSeparator::calculate_other_agent_coeff(const Size32 num_es,
                                                                         const Edge* es,
                                                                         const Time t,
                                                                         const Path& path)
{
