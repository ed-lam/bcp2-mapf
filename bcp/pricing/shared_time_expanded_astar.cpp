...
SharedTimeExpandedAStar::SharedTimeExpandedAStar(const Instance& instance, Problem& problem,
...
template <Bool feasible, Bool towards_end>
void SharedTimeExpandedAStar::generate_nodetime(const NodeTime next_nt, Label<feasible>* current,
                                                const Direction d, const Cost edgetime_penalty)
{
    // ZoneScopedC(TRACY_COLOUR);
    ...
    if (is_ge(cost_f, 0.0))
    {
        ...
        if (verbose)
        {
            ...
            {
                PRINTLN(
                    "        Cost infeasible nt {}, n {}, xyt {}, f {}, g {}, once-off {}, "
                    "rectangle {}",
                    next_nt.id(),
                    next_n,
                    format_nodetime(next_nt, map_),
            ...
            {
                PRINTLN(
                    "        Cost infeasible nt {}, n {}, xyt {}, g {}, once-off {}, rectangle {}, "
                    "time f {}, time g {}",
                    next_nt.id(),
                    next_n,
                    format_nodetime(next_nt, map_),
...
void SharedTimeExpandedAStar::generate_end(Label<feasible>* current)
{
    ...
    if (is_ge(next->g, 0.0))
    {
        ...
        if (verbose)
        {
            ...
            {
                PRINTLN(
                    "        Cost infeasible at end nt {}, n {}, xyt {}, g {}, once-off {}, "
                    "rectangle {}",
                    next_nt.id(),
                    next_nt.n,
                    format_nodetime(next_nt, map_),
...
void SharedTimeExpandedAStar::expand(Label<feasible>* current)
{
        ...
        next_t <= latest_visit_time_[next_n] && edgetime_penalties.north < COST_INF)
    {
        ...
        generate_nodetime<feasible, towards_end>(
        ...
        next_t <= latest_visit_time_[next_n] && edgetime_penalties.west < COST_INF)
    {
        const NodeTime next_nt{next_n, next_t};
        generate_nodetime<feasible, towards_end>(
            next_nt, current, Direction::WEST, edgetime_penalties.west);
    }
        ...
        next_t <= latest_visit_time_[next_n] && edgetime_penalties.east < COST_INF)
    {
        const NodeTime next_nt{next_n, next_t};
        generate_nodetime<feasible, towards_end>(
            next_nt, current, Direction::EAST, edgetime_penalties.east);
    }
        ...
        next_t <= latest_visit_time_[next_n] && edgetime_penalties.wait < COST_INF)
    {
        const NodeTime next_nt{next_n, next_t};
        generate_nodetime<feasible, towards_end>(
            next_nt, current, Direction::WAIT, edgetime_penalties.wait);
    }
...
SharedTimeExpandedAStar::Label<feasible>* SharedTimeExpandedAStar::push(Label<feasible>* next)
{
    ...
    for (auto existing = *insert_prev_next; existing; existing = *insert_prev_next)
    {
        ...
        // Calculate the potential cost of the existing label if it incurred the same penalties as
        // the new label. If the existing label still costs less than or equal to the new label,
        // even after incurring these penalties, then the new label is dominated.
...
template <Bool feasible>
Cost SharedTimeExpandedAStar::solve(const Agent a, Vector<NodeTime>& waypoints,
                                    ...
                                    const Time earliest_target_time, const Time latest_target_time)
{
    ...
    if (verbose)
    {
        edgetime_penalties.print_outgoing(map_);
    }
    if (verbose)
    {
        finish_time_penalties.print();
    }
    if (verbose)
    {
...
template Cost SharedTimeExpandedAStar::solve<true>(
    const Agent a, Vector<NodeTime>& waypoints, const Real64 constant,
    const EdgeTimePenalties& edgetime_penalties, FinishTimePenalties& finish_time_penalties,
    const OnceOffPenalties& once_off_penalties, const RectanglePenalties& rectangle_penalties,
    const Time* const latest_visit_time, const Time earliest_target_time,
    const Time latest_target_time);
template Cost SharedTimeExpandedAStar::solve<false>(
    const Agent a, Vector<NodeTime>& waypoints, const Real64 constant,
    const EdgeTimePenalties& edgetime_penalties, FinishTimePenalties& finish_time_penalties,
    const OnceOffPenalties& once_off_penalties, const RectanglePenalties& rectangle_penalties,
    const Time* const latest_visit_time, const Time earliest_target_time,
