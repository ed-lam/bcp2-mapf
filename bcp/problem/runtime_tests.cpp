...
static_assert(sizeof(FlexibleArrayTestStruct) == sizeof(Agent) * 2,
              "Your compiler or platform does not support flexible array "
              "members and/or arrays of size 0.");
...
void test_flexible_array_pointers()
{
    ...
    ASSERT(reinterpret_cast<std::uintptr_t>(&ptr->a3) ==
               reinterpret_cast<std::uintptr_t>(&ptr->data),
           "Your compiler or platform does not support flexible array members "
           "and/or arrays of size 0.");
    ASSERT(reinterpret_cast<std::uintptr_t>(&ptr->array) ==
